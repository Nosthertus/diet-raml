...
Generator.prototype.$config = JSON.parse(fs.readFileSync(__dirname + "/../config.json"));
...
Generator.prototype.executeFunction = function(func) {
	switch(func){
		...
		case "index":
			...
			break;
		default:
			console.error("ERROR: \""+func+"\" generation function does not exist")
			this.emit("done", func);
			this.emit("done."+func);
			break
	}
...
Generator.prototype.generateTraits = function() {
	...
	if(Object.keys(traits).length == 0){
		console.warn("WARNING: Traits not found in RAML");
...
Generator.prototype.generateIndex = function(){
	...
	script.directory = self.directory;
	script.setName('index');

	script.addContent(coder.loadTemplate('index_handler'));
